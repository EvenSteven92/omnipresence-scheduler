...
export const Route = createFileRoute("/clients")({
  head: () => ({
    meta: [
      { title: "Clients — OmniPresence" },
      {
        name: "description",
        content: "Connect accounts, arm auto-post, and manage TORCC, First Love, Open Eyes, and KEKA.",
      },
    ],
  }),
  ...
  return (
    <div>
      <PageHeader
        eyebrow="Clients"
        title="Your clients"
        description={`Personal ops for ${workspace.name}: connections, armed auto-post, kill switch.`}
        actions={
          <>
            <Link to="/queue" className="btn-action btn-action-secondary">
      ...
      <div className="page-content mx-auto max-w-[1320px] space-y-6">
        ...
        >
          <span className="font-semibold text-foreground">Personal tool</span> — four clients on this
          Mac. Schedules and boards stay in local storage. Armed auto-post needs the local worker
          (coming next).
        </div>
        {banner ? (
          <div
          ...
          <aside className="page-grid-rail space-y-4">
            ...
            >
              ...
              <p className="mt-4 text-body-sm leading-relaxed text-muted-foreground">
                Metrics, schedules, and connections stay scoped to this client. Switch brands below
                or from the sidebar.
              </p>
              <div className="mt-4 space-y-2 border-t border-line pt-4">
                <p className="font-mono text-[0.55rem] font-bold uppercase tracking-[0.1em] text-muted-foreground">
                  Auto-post · {ops.autoPublish ? "armed" : "held"}
                </p>
                <label className="flex items-center justify-between gap-3 text-sm">
                  <span>Arm this client</span>
                  <input
                    type="checkbox"
                    checked={ops.armed}
                    onChange={(e) => ops.setArmed(e.target.checked)}
                    className="h-4 w-4"
                  />
                </label>
                <label className="flex items-center justify-between gap-3 text-sm">
                  <span>Kill switch (pause publishes)</span>
                  <input
                    type="checkbox"
                    checked={ops.paused}
                    onChange={(e) => ops.setPaused(e.target.checked)}
                    className="h-4 w-4"
                  />
                </label>
                <label className="flex items-center justify-between gap-3 text-sm">
                  <span>Master armed (all clients)</span>
                  <input
                    type="checkbox"
                    checked={ops.globalArmed}
                    onChange={(e) => ops.setMasterArmed(e.target.checked)}
                    className="h-4 w-4"
                  />
                </label>
              </div>
            ...
            <section className="rounded-lg border border-line bg-card p-4">
              <div className="mb-3 flex items-center justify-between gap-2">
                <h2 className="font-display text-sm font-bold text-foreground">Your clients</h2>
            ...
            <section className="rounded-lg border border-line bg-paper-2 p-5">
              ...
              <ol className="mt-3 space-y-2.5 text-body-sm leading-relaxed text-muted-foreground">
                ...
                <li>
                  ...
                  Composer; each client stays isolated.
                </li>
                <li>
