...
export function ComposerPreviewRail({
  ...
  return (
    <aside className="composer-preview-pane p-5" data-testid="composer-preview-rail">
      <div className="sticky top-0 flex flex-col gap-5">
        ...
        <div>
          ...
          <div
            className="mt-3 overflow-hidden rounded-lg border-[1.5px] border-foreground bg-card"
            data-testid="composer-feed-preview"
          >
            <div className="flex items-center gap-2.5 border-b-[1.5px] border-foreground/15 px-3 py-2.5">
              <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full border-[1.5px] border-foreground bg-accent font-display text-xs font-bold text-foreground">
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="font-mono text-[0.5625rem] font-semibold uppercase tracking-[0.08em] text-muted-foreground">
                  Instagram
                </div>
              </div>
            </div>
            <div className="relative aspect-square w-full bg-background">
              {draft.previewUrl ? (
                <CardThumbnail
                  ...
                  post={{ id: draft.id, title: draftDisplayTitle(draft), mediaKind: draft.mediaKind }}
                  alt={draft.filename}
            ...
            <div className="px-3 py-3 text-sm leading-relaxed text-foreground">
              ...
              {hashtags ? (
                <span className={cn(hashtags && "text-accent")}>{hashtags}</span>
              ) : null}
              {!caption && !hashtags ? (
